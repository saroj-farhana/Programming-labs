...
[File_Name Path_Name]=uigetfile({'*.jpg'},'Select Base Image');
...
a1=rgb2gray(a1);
%ftrs=detectMSERFeatures(lefteye2);
%imshow(lefteye2);hold on; plot(ftrs);
righteye2=bbox(:, 9:12);
nose2=bbox(:,17:20);
b1=imcrop(F,nose2);
b1=rgb2gray(b1);
...
figure, imshow(a), title('Detected Eyepoints'); hold on; 
plot(eyepts1);
...
figure, imshow(b), title('Detected Nose points'); hold on; 
plot(nose1);
...
figure, imshow(c1), title('Detected Lip points'); hold on; 
plot(mouth2);
